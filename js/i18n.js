/* ----------  Cultoko — i18n  ---------- */
const I18N = {
  fr: {
    appTitle: 'Cultoko',
    tagline: 'Mini-jeux de culture générale',
    continue: 'Continuer',
    points: 'points',
    point: 'point',
    bank: 'Banque',
    shop: 'Boutique',
    shopTitle: 'Boutique des skins',
    shopDesc: 'Gagne 1 point toutes les 10 bonnes réponses, achète des skins ici.',
    skinFree: 'Gratuit',
    skinOwn: 'Possédé',
    skinEquip: 'Équiper',
    skinEquipped: 'Équipé',
    skinBuy: 'Acheter',
    skinTooPoor: 'Pas assez de points',
    home: 'Accueil',

    chooseTheme: 'Choisis un thème',
    chooseMode: 'Choisis un mode de jeu',
    chooseLevel: 'Choisis une difficulté',
    modes: {
      quiz: 'Quiz classique',
      timeAttack: 'Contre-la-montre',
      trueFalse: 'Vrai ou Faux',
      memory: 'Memory / Association',
      guessImage: "Devine l'image"
    },
    modesSub: {
      quiz: '20 questions',
      timeAttack: '60 sec, max de bonnes réponses',
      trueFalse: 'Réponds le plus vite possible',
      memory: 'Trouve les paires',
      guessImage: 'Identifie l\'image'
    },
    levels: { easy: 'Facile', medium: 'Moyen', hard: 'Difficile', expert: 'Expert' },
    question: 'Question',
    of: 'sur',
    score: 'Score',
    finish: 'Résultat',
    next: 'Question suivante',
    playAgain: 'Rejouer',
    home: 'Accueil',
    backToThemes: 'Choisir un autre thème',
    backToModes: 'Choisir un autre mode',
    correct: 'Bravo !',
    wrong: 'Raté !',
    excellent: 'Excellent !',
    nice: 'Bien joué !',
    keepGoing: 'Continue tes efforts !',
    tryAgain: 'Réessaie !',
    perfect: 'PARFAIT !',
    time: 'Temps',
    seconds: 's',
    true: 'VRAI',
    false: 'FAUX',
    correctAnswers: 'bonnes réponses',
    pairs: 'Paires trouvées',
    notAvailable: 'Pas encore disponible pour ce thème',
    comingSoon: 'Bientôt disponible',
    starter: 'Contenu starter',
    starterDesc: 'Ce thème a peu de questions pour l\'instant. Ajoute-en facilement dans data/' ,
        profile: 'Profil',
    profileTitle: 'Mon profil',
    signInTitle: 'Connecte-toi',
    signInDesc: 'Crée ton profil pour personnaliser ton expérience.',
    signInGoogle: 'Continuer avec Google',
    signInGoogleHint: 'Le bouton Google nécessite une configuration. Consulte ACCOUNT_SETUP.md.',
    signInManual: 'Ou crée un profil local :',
    namePlaceholder: 'Ton nom (ex: Grilou)',
    emailPlaceholder: 'Ton email (optionnel)',
    signInBtn: 'Créer mon profil',
    signOutBtn: 'Se déconnecter',
    signedInAs: 'Connecté en tant que',
    syncNote: 'Note : pour l\'instant tes points sont stockés localement. La synchro entre appareils arrive bientôt.',
    install: 'Installer Cultoko sur ton appareil',
    installBtn: 'Installer',
    moreCountries: 'Note : pour voir les drapeaux, une connexion internet est nécessaire au 1er affichage.'
  },
  en: {
    appTitle: 'Cultoko',
    tagline: 'General-knowledge mini-games',
    continue: 'Continue',
    points: 'pts',
    point: 'pt',
    bank: 'Bank',
    shop: 'Shop',
    shopTitle: 'Skin shop',
    shopDesc: 'Earn 1 point every 10 correct answers. Spend them here.',
    skinFree: 'Free',
    skinOwn: 'Owned',
    skinEquip: 'Equip',
    skinEquipped: 'Equipped',
    skinBuy: 'Buy',
    skinTooPoor: 'Not enough points',
    home: 'Home',

    chooseTheme: 'Choose a theme',
    chooseMode: 'Choose a game mode',
    chooseLevel: 'Choose a difficulty',
    modes: {
      quiz: 'Classic Quiz',
      timeAttack: 'Time Attack',
      trueFalse: 'True or False',
      memory: 'Memory / Pairs',
      guessImage: 'Guess the Image'
    },
    modesSub: {
      quiz: '20 questions',
      timeAttack: '60 sec, score as much as possible',
      trueFalse: 'Answer as fast as you can',
      memory: 'Find matching pairs',
      guessImage: 'Identify the image'
    },
    levels: { easy: 'Easy', medium: 'Medium', hard: 'Hard', expert: 'Expert' },
    question: 'Question',
    of: 'of',
    score: 'Score',
    finish: 'Result',
    next: 'Next question',
    playAgain: 'Play again',
    home: 'Home',
    backToThemes: 'Pick another theme',
    backToModes: 'Pick another mode',
    correct: 'Correct!',
    wrong: 'Wrong!',
    excellent: 'Excellent!',
    nice: 'Nicely done!',
    keepGoing: 'Keep going!',
    tryAgain: 'Try again!',
    perfect: 'PERFECT!',
    time: 'Time',
    seconds: 's',
    true: 'TRUE',
    false: 'FALSE',
    correctAnswers: 'correct answers',
    pairs: 'Pairs found',
    notAvailable: 'Not available yet for this theme',
    comingSoon: 'Coming soon',
    starter: 'Starter content',
    starterDesc: 'This theme has few questions for now. Add more in data/',
        profile: 'Profile',
    profileTitle: 'My profile',
    signInTitle: 'Sign in',
    signInDesc: 'Create your profile to personalize your experience.',
    signInGoogle: 'Continue with Google',
    signInGoogleHint: 'Google button requires setup. Check ACCOUNT_SETUP.md.',
    signInManual: 'Or create a local profile:',
    namePlaceholder: 'Your name (e.g. Grilou)',
    emailPlaceholder: 'Your email (optional)',
    signInBtn: 'Create my profile',
    signOutBtn: 'Sign out',
    signedInAs: 'Signed in as',
    syncNote: 'Note: for now your points are stored locally. Cross-device sync coming soon.',
    install: 'Install Cultoko on your device',
    installBtn: 'Install',
    moreCountries: 'Note: an internet connection is needed the first time to load flag images.'
  }
};

const Lang = {
  current: localStorage.getItem('cultura_lang') || (navigator.language.startsWith('en') ? 'en' : 'fr'),
  set(l) {
    this.current = l;
    localStorage.setItem('cultura_lang', l);
  },
  t(key) {
    const parts = key.split('.');
    let v = I18N[this.current];
    for (const p of parts) v = v?.[p];
    return v ?? key;
  },
  // Get a localized value from a {fr, en} object or a string
  pick(val) {
    if (val == null) return '';
    if (typeof val === 'string') return val;
    return val[this.current] || val.fr || val.en || '';
  }
};
